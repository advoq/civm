...
> `advoq/civm#226`; a adoção do consumidor Advoq ocorre no PR dele, após este
...
- **Inferência / proposta:** se o gate consumidor não estiver adotado em um
  ...
  peer; Advoq é o canário obrigatório.
...
6. **Inferência / proposta:** atualizar runbooks/paridade e o gate Advoq
   estático; executar testes locais, CI do Civm, deploy controlado e PR Advoq
...
- **CA-6:** o workflow Advoq usa runner `civm-gate` estático e o gate calcula
...
- **Confirmado por decisão do operador:** somente depois do deploy, o PR único
  do Advoq dispara a validação black-box limpa com múltiplos seeds/carga; o
