...
| DT-v2-19 | **HIGH — valor de `DefaultHostVolumeHeadroomGB=15` sem base** | A primeira execução offline (ITEM-12) roda `Optimize-VHD` com `Start-Transcript` e poll de `v_free` a cada 5 s; registra o **low-water mark** de scratch. Se < 15 GB insuficiente, elevar o constante e registrar. |
