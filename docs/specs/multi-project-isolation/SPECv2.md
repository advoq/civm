...
Avaliado sobre o **primeiro conjunto de 5 runs consecutivos de CADA peer (advoq, vitae, compexhub) nas primeiras 48h** após o deploy do binário. Reverter a slice ofensora (`self-upgrade` versão anterior + `rm /var/lib/civm/port-blocks.json`) se **qualquer**:
