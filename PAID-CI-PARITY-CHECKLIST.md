...
| --- | --- | --- | --- | --- | --- |
| 1 | **Ephemerality / clean-slate** | VM nova e destruída por job; zero estado herdado | Runner long-lived; só `_work` velho apagado + caches trimados no `job-completed`; **volumes/imagens de runs anteriores persistem** (106 volumes, imagens `acme-<runId>-*` acumuladas) | **INFIEL** | Hook de `job-started` que faz `docker volume prune` dos volumes órfãos de runs concluídas (label/project-name por runId) + `compose down -v` garantido; ver D-slot/ITEM-12. Clean-slate total = IMPOSSÍVEL (sem VM-por-job) |
