...
1. **Provisionar o gate runner** (host, elevado):
   ...
   $adminToken -Url https://github.com/advoq -Index 1`. O provisionador cria o
   ...
   'orgs/advoq/actions/runners?per_page=100' --jq '.runners[] | select(.name ==
   "civm-advoq-gate-1") | {name,busy,labels}'`.
